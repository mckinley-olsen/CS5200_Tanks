Changes to Documentation:
	attributes added to RegisterReply message: gameMapMaxX, gameMapMaxY, startingLocation
	attributes added to LocationListReply message: playerID
	additional requirement, #12, that describes the requirements in using the webservice
	additional section under Structure that describes the structure of the statistics implementation
	

Testing Added:
	StatsManager testing: included in this test file are all cases to test the webservice functionality
		this test file is within the TanksFightManager project, under test packages
	
	For advanced requirements:
		UnregisterRequest 	- 	contains 7 test cases
		AckNak 				-	contains 3 test cases
		LocationListRequest	- 	contains 5 test cases
		LocationListReply	-	contains 7 test cases
		LastLocationsRequest-	contains 3 test cases
		CreateFightReply	-	contains 5 test cases
		FireShellReply		-	contains 7 test cases
		total new test cases: 37

Changes to UI in Client:
	Almost nothing. I've begun to draw a game map, but it's hardly functional.
	
Additional protocols implemented: (this should be close to 6/10)
	LocationList
	Unregister
	
Readme from HW3, as it still applies:
