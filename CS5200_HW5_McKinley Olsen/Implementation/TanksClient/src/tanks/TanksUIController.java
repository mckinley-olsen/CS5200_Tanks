...
import Strategy.FightListStrategy;
...
import Strategy.LocationListStrategy;
import Strategy.PlayerListStrategy;
import Strategy.RegisterStrategy;
import Strategy.Strategy;
import TanksCommon.Communicator;
import Strategy.Task;
import Strategy.UnregisterStrategy;
...
public class TanksUIController extends Controller implements Initializable
{
    ...
    {
        Strategy s = new PlayerListStrategy(0);
        Task task = new Task(s);
        TanksUIController.getDoer(MAIN_DOER_NUMBER).addTask(task);
    }
    @FXML
    ...
    {
        Strategy s = new FightListStrategy();
        Task task = new Task(s);
        TanksUIController.getDoer(MAIN_DOER_NUMBER).addTask(task);
    }
    @FXML
    ...
    {
        Strategy s = new LocationListStrategy(TanksClientModel.getProcessID());
        Task task = new Task(s);
        TanksUIController.getDoer(MAIN_DOER_NUMBER).addTask(task);
    }
    @FXML
    ...
    {
        Strategy s = new UnregisterStrategy();
        Task task = new Task(s);
        TanksUIController.getDoer(MAIN_DOER_NUMBER).addTask(task);
    }
    // </editor-fold>
